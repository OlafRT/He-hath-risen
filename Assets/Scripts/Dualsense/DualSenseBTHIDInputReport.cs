// BT DualSense input report — offsets confirmed from raw HID byte capture.
//
// Windows strips the BT framing and presents a 78-byte report with ID 0x01.
// The layout is completely different from USB — buttons come BEFORE triggers:
//
//  [00]      report ID (0x01)
//  [01-02]   Left Stick X, Y
//  [03-04]   Right Stick X, Y
//  [05]      buttons1  — hat switch (bits 0-3) + face buttons (bits 4-7)
//  [06]      buttons2  — L1, R1, L2btn, R2btn, Share, Options, L3, R3
//  [07]      counter   — varies each event, ignored
//  [08]      buttons3  — PS, touchpad, mic
//  [09]      R2 analog — 0-255
//  [10]      L2 analog — 0-255 (confirmed by pattern; verify with L2 press)
//  [11+]     IMU + touchpad + battery (not needed for gameplay)
...
internal struct DualSenseBTHIDInputReport : IInputStateTypeInfo
{
    ...
    // ── Buttons (come BEFORE triggers in BT format) ───────────────────────
    ...
    // byte[7] is a counter that changes every event — no control mapped here

    // ── Triggers (confirmed from raw bytes: L2=byte8, R2=byte9) ─────────
    // buttons3 (PS/touchpad/mic) is at an unknown higher offset — not used in game.
    [InputControl(name = "leftTrigger",  format = "BYTE")] [FieldOffset(8)]  public byte leftTrigger;
    [InputControl(name = "rightTrigger", format = "BYTE")] [FieldOffset(9)]  public byte rightTrigger;
}
